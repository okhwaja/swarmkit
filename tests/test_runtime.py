...
class RuntimeTest(unittest.TestCase):
    ...
    def test_cancel_is_terminal_and_preserves_uncertain_actions(self):
        ...
        effect = s.prepare_effect(
            self.conn, task, "worker", "merge:abc", "PR/1", "abc", {"merge": True}
        )
    ...
    def test_effect_replay_is_deduplicated_and_uncertainty_blocks_reclaim(self):
        ...
        s.transition_effect(
            self.conn, effect["id"], "succeeded", "human", "Provider receipt 123 for abc"
        )
        self.claim(task, "fresh")
        self.assertEqual(
            s.prepare_effect(self.conn, task, "fresh", "approve:abc", "PR/1", "abc", {})["state"],
            "SUCCEEDED",
        )
    ...
    def test_current_evidence_rejects_wrong_revision_and_tampering(self):
        ...
        self.assertEqual(
            self.conn.execute(
                "SELECT COUNT(*) FROM artifacts WHERE task_id=? AND kind='verification'", (task,)
            ).fetchone()[0],
            2,
        )
    ...
    def test_confirmation_survives_restart_and_requires_current_ack(self):
        ...
        decision = s.block_task(
            self.conn, task, "worker", "human_decision", "Proceed?", "Approve", ["yes", "no"]
        )
    ...
    def test_amendment_invalidates_old_plan_and_keeps_history(self):
        ...
        s.amend_mission(
            self.conn, "New objective", ["New criterion"], ["Bounded"], "Changed plans", "human"
        )
        self.assertEqual(s.runtime_state(self.conn)["revision"], 2)
        self.assertEqual(s.task_row(self.conn, task)["authorized"], 0)
        self.assertEqual(
            self.conn.execute("SELECT COUNT(*) FROM mission_amendments").fetchone()[0], 1
        )
    ...
    def test_failed_migration_rolls_back_schema_and_version(self):
        ...
        self.conn.execute("UPDATE meta SET value='6' WHERE key='schema_version'")
        self.conn.commit()
        ...
        self.assertIsNone(
            self.conn.execute("SELECT name FROM sqlite_master WHERE name='attempts'").fetchone()
        )
        self.assertEqual(
            self.conn.execute("SELECT value FROM meta WHERE key='schema_version'").fetchone()[0],
            "6",
    ...
    def test_audit_database_and_views_match_and_privacy_allowlist(self):
        ...
        with zipfile.ZipFile(output) as archive:
            ...
            raw_db = self.base / "audit.sqlite3"
            raw_db.write_bytes(archive.read("swarm-audit/state.sqlite3"))
        with sqlite3.connect(raw_db) as conn:
            self.assertEqual(
                conn.execute("SELECT MAX(seq) FROM events").fetchone()[0],
                manifest["event_watermark"],
            )
        s.export_audit(self.root, output, share_safe=True)
        with zipfile.ZipFile(output) as archive:
            self.assertEqual(
                set(archive.namelist()), {"swarm-audit/manifest.json", "swarm-audit/telemetry.json"}
            )
            self.assertNotIn(
                str(self.root), "".join(archive.read(n).decode() for n in archive.namelist())
            )
    ...
    def test_controller_crash_preserves_child_lock_until_harness_exits(self):
        ...
        self.config(
            [
                sys.executable,
                "-c",
                "from pathlib import Path; import time; Path(%r).touch(); time.sleep(1.5)"
                % str(marker),
            ]
        )
    ...
    def test_manager_lease_expiration_does_not_touch_task_attempts(self):
        ...
        self.assertEqual(
            self.conn.execute(
                "SELECT status FROM manager_reviews WHERE id=?", (review_id,)
            ).fetchone()[0],
            "PENDING",
        )
    ...
    def test_orphaned_manager_reopens_review_without_waiting_for_lease(self):
        ...
        self.conn.commit()
        s.claim_manager_review(self.conn, "manager", 3600)
        self.conn.execute(
    ...
    def test_policy_fanout_cannot_bypass_task_quota(self):
        ...
        with self.assertRaisesRegex(s.SwarmError, "quota"):
            s.apply_policy(
                self.conn, "system-port", ["goal=port", "test_command=test"], None, "manager", True
            )
        self.assertEqual(self.conn.execute("SELECT COUNT(*) FROM tasks").fetchone()[0], 0)
        self.assertEqual(
            self.conn.execute("SELECT COUNT(*) FROM policy_applications").fetchone()[0], 0
        )
    ...
    def test_all_new_templates_apply_executable_dependency_graphs(self):
        for name in ("performance-investigation", "pipeline-repair", "system-port"):
            ...
            s.install_policy(self.conn, source, "human")
            result = s.apply_policy(
                self.conn, name, ["goal=bounded goal", "test_command=test"], None, "manager", True
            )
            self.assertTrue(result)
        self.assertEqual(
            self.conn.execute("SELECT COUNT(*) FROM policy_applications").fetchone()[0], 3
        )
    ...
    def test_cancel_dependency_cancels_transitive_reducers(self):
        first = self.task()
        second = s.add_task(
            self.conn, "Middle", "Reduce", "verification", ["Checked"], [first], 50, "manager", True
        )
        last = s.add_task(
            self.conn, "Last", "Reduce", "verification", ["Checked"], [second], 50, "manager", True
        )
    ...
    def test_amendment_deauthorizes_confirmation_waits(self):
        ...
        self.claim(task)
        decision = s.block_task(
            self.conn, task, "worker", "human_decision", "Proceed?", "yes", ["yes", "no"]
        )
    ...
    def test_audit_verify_cli_needs_no_mission_and_fails_for_corruption(self):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(
                s.main(["--root", str(self.base / "absent"), "audit-verify", str(output)]), 0
            )
        with zipfile.ZipFile(output, "a") as archive:
            archive.writestr("extra", "corrupt")
        with contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(
                s.main(["--root", str(self.base / "absent"), "audit-verify", str(output)]), 2
            )

    def test_exhausted_high_priority_task_does_not_starve_eligible_work(self):
        first, eligible = self.task(), self.task()
        for n in range(3):
            self.claim(first, "failed-%d" % n)
            self.expire(first)
        self.config([sys.executable, "-c", "pass"])
        path = self.root / "runner.json"
    ...
    def test_runtime_cli_end_to_end(self):
        ...
        for command in (
            ["why"],
            ["inbox", "--agent", "reader", "--lease"],
            ["configure", "--limits", '{"max_tasks":5}'],
        ):
            with contextlib.redirect_stdout(io.StringIO()):
